...
| 2026-05-27 | Two slides cut by user (headline-quote slide and the dedicated three-thinkers slide); figures refreshed. Swapped `Figures/verdicts.svg` (Icelandic, deleted) → `Figures/verdicts.png` (English re-cut, 2400×1320). Added a topics slide right after verdicts using the new `Figures/topics.png` — mirrors the Nordic House Gæði → Umræðan order (verdicts then topics). Both bumped to `width="86%"` since the new PNGs are wider than the old SVG. `R/` and `data/` checked in — the make_figures.R / prototype_completeness.R / save_summary_csvs.R scripts that produce the figures from the verdict / topic / headline summary CSVs | User regenerated the charts from live esbvaktin.is data in English; previous SVG was Icelandic and stale |
| 2026-05-27 | Three-tips slide (now slide 11, second-to-last after the cuts) re-laid out with portrait thumbnails. Each `.tip-row` is now a CSS grid (`110px 1fr`): circular 95px portrait on the left, italic Fraunces quote + Jökull-teal attribution on the right. Photos: `timhaford.webp` (634×766), `davidspiegelhalter.jpg` (480×571), `onoraoneill.jpg` (1537×2048). `object-fit: cover` + `object-position: center top` normalises the variable source aspect ratios to consistent circles. Each row still fragments in one at a time | User asked for thinker photos alongside the quotes — gives each thinker a visual anchor without overcrowding the slide |
| 2026-05-27 | Added a Þingfréttir viz-gallery slide (between the chart+Miðeind slide and the ESB-vaktin section). Six `Figures/thingfrettir_viz1.png`–`thingfrettir_viz6.png` images stacked in a `.image-stack` div, each marked `.fragment` so they reveal one at a time. CSS uses absolute positioning with `top:50% left:50%` + `translate(-50%, -50%)` — the last revealed fragment sits on top in z-order, so it reads as "one image flipped to the next". Each image gets its own card treatment (white bg, border, shadow) so the layering looks intentional when earlier images peek out at the edges. `.image-stack > p { margin: 0 }` to neutralise Quarto's `<p>` wrappers around each image so they don't add flow space inside the absolute stack | User wanted a fast walk-through of more Þingfréttir visualisations — "quickly go through them just to show them lightly" |
| 2026-05-27 | Slide 5 (Þingfréttir chart-and-callout) rebuilt as Icelandic article + "Getting Icelandic right" steps. Image swap: `thingfrettir-charts.png` → `thingfrettir-article-icelandic.png` (an Icelandic-language Þingfréttir article — proof that the pipeline produces good Icelandic). Sidebar replaced: the old `.chart-callout` (label + big "Miðeind" name) is gone; new `.review-steps` block shows a numbered list of the 3 tricks the user learned for good Icelandic — 1. Access to raw texts, 2. No English in context, 3. Miðeind tools. CSS counters give Jökull-teal numbers; Fraunces step text in basalt; same Jökull left-border accent for visual continuity with other callouts in the deck | User's instruction: "exchange the thingfrettir-charts.png screenshot for thingfrettir-article-icelandic.png" + capture the 3 Icelandic-quality tricks as sidebar steps. The Miðeind reference is now folded into the steps list (step 3) instead of being the visual punchline |
